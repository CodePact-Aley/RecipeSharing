...
export const createUser = async () => {
    try {
        ...
        const password_hash = process.argv[4]; // Second command-line argument as password hash
        const name = process.argv[5]; // Third command-line argument as name
        const email = process.argv[6]; // Fourth command-line argument as email

        // Check if all required fields are provided
        if (!username || !password_hash || !name || !email) {
            throw new Error('Please provide username, password hash, name, and email as command-line arguments.');
        }
